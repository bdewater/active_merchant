...
class GatewayTest < Test::Unit::TestCase
  ...
  def test_amount_does_not_modify_most_currencies
    Gateway.money_format = :dollars
    assert_equal '1.00', @gateway.send(:amount, 100, 'CAD')
    assert_equal '12.34', @gateway.send(:amount, 1234, 'USD')

    Gateway.money_format = :cents
    assert_equal '100', @gateway.send(:amount, 100, 'CAD')
    assert_equal '1234', @gateway.send(:amount, 1234, 'USD')
  end

  def test_amount_ignores_money_format_for_non_fractional_currencies
    Gateway.money_format = :dollars
    assert_equal '1', @gateway.send(:amount, 100, 'JPY')
    assert_equal '12', @gateway.send(:amount, 1234, 'ISK')
  ...
  def test_amount_returns_three_decimal_places_for_three_decimal_currencies
    Gateway.money_format = :dollars
    assert_equal '0.100', @gateway.send(:amount, 100, 'OMR')
    assert_equal '1.234', @gateway.send(:amount, 1234, 'BHD')

    Gateway.money_format = :cents
    assert_equal '100', @gateway.send(:amount, 100, 'OMR')
    assert_equal '1234', @gateway.send(:amount, 1234, 'BHD')
  end
